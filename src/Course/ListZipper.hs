...
fromList ::
  ...
  -> MaybeListZipper a
fromList Nil = IsNotZ
fromList (a:.rest) = IsZ $ ListZipper Nil a rest
...
findLeft ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findLeft pred (ListZipper l a r) =
    case found of
        Nil -> IsNotZ
        (a':.rest) -> IsZ $ ListZipper rest a' (reverse l'++(a:.r))
    where
        (l', found) = break pred l
...
findRight ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findRight pred (ListZipper l a r) =
    case found of
        Nil -> IsNotZ
        (a' :. rest) -> IsZ $ ListZipper (reverse r'++ a:.l) a' rest
    where
        (r', found) = break pred r
...
moveLeftLoop ::
  ListZipper a
  -> ListZipper a
moveLeftLoop lz@(ListZipper Nil a Nil) =
    lz
moveLeftLoop (ListZipper Nil a r) = let
    (lst:.rest) = reverse (a:.r)
    in ListZipper rest lst Nil
moveLeftLoop (ListZipper (a':.l) a r) =
    ListZipper l a' ( a:.r )
...
moveRightLoop ::
  ListZipper a
  -> ListZipper a
moveRightLoop lz@(ListZipper Nil a Nil) = lz
moveRightLoop (ListZipper l a Nil) = let
    (nxt:.rest) = reverse (a:.l)
    in ListZipper Nil nxt rest
moveRightLoop (ListZipper l a (a':.r)) =
    ListZipper (a:.l) a' r
...
swapLeft (ListZipper (x:.l) a r) = IsZ $ ListZipper (a:.l) x r
...
swapRight (ListZipper l a (x:.r)) = IsZ $ ListZipper l x (a:.r)
...
moveLeftN 1 lz = moveLeft lz
moveLeftN n lz
    | n < 0 = moveRightN (abs n) lz
    | otherwise = moveLeftN (n-1) (moveLeftLoop lz)
...
moveLeftN' n lz@(ListZipper l a r)
    ...
    | n < 0 = moveRightN' (abs n) lz
    | otherwise = Right $ foldLeft (const . moveLeftLoop) lz (listh [1..n])
...
moveRightN' n lz@(ListZipper _ _ r)
    ...
    | n < 0 = moveLeftN' (abs n) lz
    | otherwise = Right $ foldLeft (const . moveRightLoop) lz $ listh [1..n]
