...
filter _ Nil = Nil
filter pred (h :. rest)
    | pred h = h :. filter pred rest
    | P.otherwise = filter pred rest
...
seqOptional (Full a :. rest) = P.fmap (a :.) $ seqOptional rest
